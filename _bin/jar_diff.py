...
def _javap_public(files):
    ...
    for chunk in _chunks(files, 200):
        results.append(
            str(subprocess.check_output(['javap', '-public'] + chunk)))
    return '\n'.join(results)
...
def _split_info_into_infos(info):
    ...
    for info in original:
        ...
        if not header.startswith('public '):
            continue
        match = re.match(
            r'(?:public |abstract |class |interface |final )+([^ ]+) .*', header)
        name = match.group(1)
        infos[name] = '\n'.join(lines)
    return infos
...
def _exec_diff(base_path, one_path, two_path):
    ...
    subprocess.call(
        ['diff', '-U', '0', '-N', one_path, two_path], cwd=base_path)
...
def _main(old_archive, new_archive, diff_script=None):
    ...
    if diff_script:
        subprocess.call('%s %s %s %s' % (
            diff_script, temp_folder, old_data, new_data), shell=True)
    else:
