    ...
    def invoke(self, args, from_tty):
        ...
        # if True:
            try:
                ...
                sys.stderr.write(
                    "error: an exception occurred during excution")
                raise
...
class HexdumpImpl(object):
    ...
    def parse_argument(self, args):
        ...
        if idx < 0:
            ...
            debug("hexdump impl parse args: |%s|, |%s|" %
                  (args[:idx].strip(), args[idx + 2:].strip()))
...
class IconvEncodings(object):
    ...
    def name(self, alias):
        ...
        alias = alias.lstrip("#")
        if alias in IconvEncodings.encodings:
    ...
    def complete(self, text, word):
        ...
        for a in IconvEncodings.encodings.keys():
            #debug("    enc(%s)" % a)
            if a.find(word) == 0:
                ret.append(a)
        return ret
...
Currently, capture the only first line, removing iconv pathname"""
    ...
    def execute_iconv(self, filename, args):
        ...
        width = max(list(map(len, encodings)))
