;;; fic-mode.el --- Show FIXME/TODO/BUG/REVIEW in special face only in comments and strings
;;--------------------------------------------------------------------
;;
...
(defun review-labeled-timestamp-comment (label)
  ...
  (insert (or comment-start "#")
          label (format-time-string " %Y-%m-%d %H:%M ") user-full-name ";  "))

(defun review-fixme-comment (&optional chooseWord)
  (interactive "P")
  (let ((commentWord "REVIEW")
        (candidates fic-highlighted-words))
    (when chooseWord
      (setq commentWord (completing-read "candidates:" candidates nil nil nil nil "FIXME" nil)))
