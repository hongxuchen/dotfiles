...
local preview_with = function(_self, content)
  ...
  if _self.filetype then
    vim.bo[tmpbuf].filetype = _self.filetype
  end
...
local github_raw_url = function(url, filepath)
  return url:gsub("github.com", "raw.githubusercontent.com"):gsub("%.git$", "") .. "/master/" .. filepath
end
...
local parse_plugin_type = function(_, plugin)
  ...
  if not vim.uv.fs_stat(dir) then
    ...
    if plugin.url:match("github") then
      return p_type.UNINS_GH
    end
...
function M.lazy:populate_preview_buf(entry_str)
  ...
  if vim.is_callable(cmdline) then
    cmdline = cmdline()
  end
