...
local all_name = actions_builder(function()
  return true
end, function(p)
  return p.name
end)
local all_repo = actions_builder(function()
  return true
end, disp_repo)

local toggle_mode = function(from_cb, to_cb, to_opts, toggle_key)
  -- note: avoid pass incorrect args to from_cb
  local go_back = { actions = {
    [toggle_key or "ctrl-g"] = function()
      return from_cb()
    end,
  } }
  ...
  local default = {
    ...
    actions = {
      ...
      ["ctrl-l"] = p_do(function(p)
        if p.dir and vim.uv.fs_stat(p.dir) then
          require("fzf-lua").files { cwd = p.dir }
        end
      end),
      ["ctrl-n"] = p_do(function(p)
        if p.dir then
          require("fzf-lua").live_grep_native { cwd = p.dir }
        end
      end),
